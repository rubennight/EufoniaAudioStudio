...
const SelectChip: React.FC <{ generos: GeneroMusical[], width: number }> = ({ generos, width = 300 }) => {
    const [selectedGeneros, setSelectedGeneros] = React.useState<string[]>([]);
    ...
    return(
            <Select 
                ...
                >
                    {generos.map((genero) => (
                        <MenuItem key={genero.id} value={genero.nombre}>
                            {genero.nombre}
                        </MenuItem>
                    ))}
            </Select>
